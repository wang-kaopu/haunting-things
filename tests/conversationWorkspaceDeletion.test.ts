...
describe('ConversationService workspace deletion', () => {
  it('stops runtimes before deleting conversations in a workspace', () => {
    ...
    const service = new ConversationService(repo as unknown, new EventBus(), '/tmp/haunting-test');
    const firstRuntime = { stop: vi.fn() };
    const secondRuntime = { stop: vi.fn() };
    (service as unknown).runtimes.set('conv-a', firstRuntime);
    (service as unknown).runtimes.set('conv-b', secondRuntime);
    ...
    expect((service as unknown).runtimes.has('conv-a')).toBe(false);
    expect((service as unknown).runtimes.has('conv-b')).toBe(false);
